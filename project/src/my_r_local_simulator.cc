...
bool MyrLocalSimulator::Start() {
  ...
  // CC Westbound stops
  // SPSC
  Stop * stop_CC_WB_1 = new Stop(4, 44.975837, -93.222174);
  // Buford & Gartner
  ...
  // before transit
  Stop * stop_CC_WB_5 = new Stop(8, 44.972392, -93.243774);
  ...
  // TODO(3081 staff): is this always true? If so, we may want to reduce
  // the length of probs to_char_type remove possibility of generating
...
bool MyrLocalSimulator::Update() {
  ...
  for (std::vector<int>::iterator it = time_since_last_bus_generation_.begin();
    ...
    if (0 >= (*bus_gen_timing_iter)) {
      ...
      // Generate SmallBus only
      bus_depot->SetStrategy(1);

      active_buses_.push_back(bus_depot->Generate(
      ...
      // Delete the bus depot instance for the current iteration
      delete bus_depot;
    } else {
      prototype_route_iter++;
      prototype_route_iter++;
